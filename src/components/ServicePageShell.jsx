...
export default function ServicePageShell({
  ...
  const onSubmit = async (e) => {
    ...
    const queryString = new URLSearchParams({
      ...
      email: form.email,
      phone: form.phone || "",
      message: `[Service: ${serviceTag || title}] ${form.message}`,
  ...
  // Slimmer Download Button Style
  const downloadBtnStyle = {
    padding: "0.6rem 2.5rem", // Reduced height (0.6rem)
    fontWeight: "500",
    fontSize: "0.95rem",
    minWidth: "240px",
    ...
    <main className="bg-accent1 section-padding" style={{ position: "relative" }}>
      ...
      <div className="wrapper" style={{ position: "relative", zIndex: 1, maxWidth: "72rem" }}>
        ...
        <div style={{ maxWidth: "56rem", marginBottom: "2.0rem" }}>
          ...
          <h1 className="heading-2 text-gradient" style={{ marginBottom: "0.75rem" }}>
            {title}
          </h1>

          <p className="desc" style={{ margin: 0, maxWidth: "52rem" }}>
            {intro}
          </p>
        ...
        {/* --- HELP GRID SECTION (THE 6 CARDS) --- */}
        {helpGrid.length > 0 && (
          <section style={{ marginBottom: "3.0rem" }}>
            <h2 style={{ 
              color: "#7c3aed", 
              textAlign: "center", 
              fontSize: "1.5rem", // Small heading as requested
              fontWeight: "700", 
              marginBottom: "1.0rem" 
            }}>
        ...
        <div className="grid-2" style={{ alignItems: "start", gap: "1.5rem" }}>
          ...
          {/* LEFT: WHAT WE PROVIDE / HOW WE HELP */}
          <section className="card" style={{ borderRadius: "1.5rem", padding: "1.9rem" }}>
            ...
            <h2 className="heading-3" style={{ margin: "1.5rem 0 0.75rem" }}>
              How we help
            </h2>
            <ul className="desc" style={{ paddingLeft: "1.1rem", margin: 0 }}>
              {helps.map((x) => (
          ...
          >
            <h2 className="heading-3" style={{ marginBottom: "0.9rem" }}>
              Request this service
            </h2>

            {!contactScriptUrl ? (
              <p className="desc">Contact form endpoint not configured.</p>
            ) : sent ? (
              <p className="desc" style={{ color: "#16a34a", fontWeight: 700 }}>
                Request sent. You will get a response soon.
              </p>
            ) : (
              <form onSubmit={onSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
                ...
                <button 
                  ...
                  style={{ 
                    ...
                    marginTop: "0.5rem",
                    padding: "0.6rem 2rem", // Slimmer height
